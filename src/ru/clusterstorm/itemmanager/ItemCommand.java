...
public class ItemCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args[0].equalsIgnoreCase("flag")) {
			...
			if(args.length == 2) {
				...
				if(args[1].equalsIgnoreCase("unbreakable")) {
					p.sendMessage(PREFIX + getItemName(item) + "§7 unbreakable§f: true");
				} else {
			...
			if(args.length > 2) {
				boolean value;
				if(args[2].equalsIgnoreCase("true") || args[2].equalsIgnoreCase("1")) value = true;
				else if(args[2].equalsIgnoreCase("false") || args[2].equalsIgnoreCase("0")) value = false;
				else {
				...
				if(args[1].equalsIgnoreCase("unbreakable")) {
					if(meta.spigot().isUnbreakable()) {
						if(value) {
							p.sendMessage(PREFIX + getItemName(item) + "§c is already unbreakable");
							return true;
						}
					} else {
						if(!value) {
							p.sendMessage(PREFIX + getItemName(item) + "§c is not unbreakable");
							return true;
						}
					}
					
					meta.spigot().setUnbreakable(value);
					if(value) p.sendMessage(PREFIX + "§aAdded flag §7unbreakable §ato §f" + getItemName(item));
					else p.sendMessage(PREFIX + "§aRemoved flag §7unbreakable §afrom §f" + getItemName(item));
				} else {
					ItemFlag f = getFlag(args[1]);
					if(f == null) {
						p.sendMessage(PREFIX + "§cUnknown flag: " + args[1]);
					...
					if(value) {
						if(meta.hasItemFlag(f)) {
							p.sendMessage(PREFIX + getItemName(item) + "§c already has " + f.name().toLowerCase());
							return true;
						}
						meta.addItemFlags(f);
						...
						if(!meta.hasItemFlag(f)) {
							p.sendMessage(PREFIX + getItemName(item) + "§c has not " + f.name().toLowerCase());
							return true;
						}
						meta.removeItemFlags(f);
						p.sendMessage(PREFIX + "§aRemoved §7" + f.name().toLowerCase() + "§a from §f" + getItemName(item));
					}
				}
			...
			if(flags.isEmpty() && !unbreakable) {
				p.sendMessage(PREFIX + getItemName(item) + "§c has no item flags");
				return true;
			}
			...
			if(unbreakable) list += "unbreakable";
			else if(!list.isEmpty()) list = list.substring(0, list.length() - 2);
		...
		if(args[0].equalsIgnoreCase("ench")) {
			...
			if(args.length < 3) {
				p.sendMessage(PREFIX + getItemName(item) + "§7: " + ench.getName().toLowerCase() + "§f: " + meta.getEnchantLevel(ench));
				return true;
			}
			...
			if(level <= 0) {
				...
				p.sendMessage(PREFIX + "§aRemoved §7" + ench.getName().toLowerCase() + "§a from§f " + getItemName(item));
			} else {
				meta.addEnchant(ench, level, true);
				p.sendMessage(PREFIX + "§aSet §7" + ench.getName().toLowerCase() + "§a level on§f " + getItemName(item) + "§a: " + level);
			}
			item.setItemMeta(meta);
			p.updateInventory();
			return true;
	...
	private static String getItemName(ItemStack item) {
		if(!item.hasItemMeta()) return getMaterialName(item);
		ItemMeta meta = item.getItemMeta();
		if(!meta.hasDisplayName()) return getMaterialName(item);
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
		if(!sender.hasPermission("itemmanager.command")) return Lists.newArrayList();
		
		if(args.length == 1) {
			return filter(Lists.newArrayList("name", "lore", "ench", "flag"), args);
		}
		if(args.length == 2) {
			if(args[0].equalsIgnoreCase("lore")) return filter(Lists.newArrayList("add", "set", "remove"), args);
			if(args[0].equalsIgnoreCase("ench")) return filter(ENCHANTMENTS, args);
			if(args[0].equalsIgnoreCase("flag")) return filter(FLAGS, args);
		}
		
		if(args.length == 3) {
			if(args[0].equalsIgnoreCase("flag")) return filter(Lists.newArrayList("true", "false"), args);
		}
